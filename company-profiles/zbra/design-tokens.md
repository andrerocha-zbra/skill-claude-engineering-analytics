> Este é o design system default da ZBRA para documentação/relatórios HTML gerados pela skill (ver `plugins/analytics-project-architect/references/06-doc-e-handoff.md` e `templates/doc/relatorio.html`). Um cliente com identidade visual própria pode sobrescrever via `@client_context/design-system/` no próprio repo-cérebro.
